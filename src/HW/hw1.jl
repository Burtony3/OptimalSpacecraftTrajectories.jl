...
sol = solve(prob, reltol=1e-13, abstol=1e-13, saveat=5); md"Integrator: Tsit5 | Tol: 1e-13 | Miss: $(norm(x₀[1:3] - sol[end][1:3])) LU"
...
begin
	...
	function EoMSTM!(du, u, p, t)
		...
		Idnt = [1 0 0; 0 1 0; 0 0 1]
		# G = μ * ( (3 * r^-5 * r⃗⊗r⃗) - (r^(-3)*Idnt) )
	...
	probb = ODEProblem(EoMSTM!, vcat(x₀, [1.0*Matrix(I, 6, 6)...]), (0.0, T), (μ))
...
genSTM(cntrDiff′, EoM_shrthnd2, x₀, sol[3], 1e-8)
