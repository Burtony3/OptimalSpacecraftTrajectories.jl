...
# ╔═╡ d0832152-1be1-11ec-138f-8da58ba0db0d
begin
	using Symbolics, DifferentialEquations, LinearAlgebra, Plots
	import PlutoUI
...
# ╔═╡ 71ab8d4f-877b-42d8-a580-72610c17b577
begin
	J(x) = -(1.4 .- 3.0*x) .* sin.(18.0*x)
	xspan = LinRange(0, 1.2, 500)
	
	# J(x) = -(x .+ sin.(x)).*exp.(-x.^2)
	# xspan = LinRange(-10, 10, 500)
end

# ╔═╡ b9deb623-7738-41a0-a4d1-e7d4b5ada708
md"## X = $(@bind a PlutoUI.Slider(LinRange(0, 1.2, 301)))"

# ╔═╡ ffe29e2f-1865-47d8-b8e2-0805e424a358
md"## h = $(@bind n PlutoUI.Slider(-16:-1))"

# ╔═╡ a898bb99-a601-49e7-b8f1-44190492839b
begin
		# xlim=(0, 1.2), xticks=0:0.15:1.2, yticks=-1.5:0.5:2)
	plt = plot(xspan, J(xspan), label="Function", title="7 Point Central Difference for h = 1e$(n)")
	
	# FINDING FIRST DERIVATIVE
	h = 10^Float64(n)
	∂J = OST.cntrDiff[7][1](J, a, h)
	∂²J = OST.cntrDiff[7][2](J, a, h)
	∂³J = OST.cntrDiff[7][3](J, a, h)
	plot!(plt, xspan, ∂J*(xspan .- a) .+ J(a), 
		label="1st = $(round(∂J, digits=2))", ylims=ylims(plt))
	plot!(plt, xspan, ∂²J*(xspan .- a).^2 .+ ∂J*(xspan .- a) .+ J(a), 
		label="2nd = $(round(∂²J, digits=2))", ylims=ylims(plt))
	plot!(plt, xspan, ∂³J*(xspan .- a).^3 .+∂²J*(xspan .- a).^2 .+ ∂J*(xspan .- a) .+ J(a), 
		label="3rd = $(round(∂³J, digits=2))", ylims=ylims(plt))
	plot!(plt, [a, a, 0], [0, J(a), J(a)], label="", color=:black, linestyle=:dash)
	plot!(plt, [a], [J(a)], label="", marker=:circle)
end
...
git-tree-sha1 = "1775a8a10940ef4b2635fb56faf47a99749e516f"
